        ...
        <el-table-column label="操作" width="300px">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              @click="shougGetRightDialod(scope.row)"
            >分配权限</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async removeById(role, rightId) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.reeor('删除权限失败')
      }
      // this.getRolesList()
      role.children = res.data
    },
    // 展开分配权限的对话框
    async shougGetRightDialod(role) {
      this.roleId = role.id
      // 获取所有权限数据
    ...
    // 通过递归的形式，获取所有角色下的三级权限对的id，并保存到defkeys中
    ...
    async allotRights() {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error('获取分配权限校角色id失败')
      }
